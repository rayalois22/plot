...
The default **width** is 640. On Observable, the width can be set to the [standard width](https://github.com/observablehq/stdlib/blob/master/README.md#width) to make responsive plots. The default **height** is 400 if the plot has a continuous *y* scale and no *fy* scale; otherwise it is 90 if the plot has an *fx* scale, or 60 if it hasn’t, to which a heuristic value is added that takes into account the number of categories if the plot has an ordinal *y* scale, as well as the number of facets if the plot has a *fy* scale.
